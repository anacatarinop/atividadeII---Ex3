...
public class Main {
    ...
    public static boolean buscaRecursiva(int[] vetor, int n,int menor,int maior) {
        if(menor > maior){
            return false;
        }
		...
		if(vetor[meio]  < n)
			return buscaRecursiva(vetor,n,meio+1,maior);
		else if(vetor[meio] > n)
			return buscaRecursiva(vetor,n,menor,meio-1);
		else
			return true;
    }

    public static boolean busca(int[] vetor, int n)
	{
		return buscaRecursiva(vetor,n,0,vetor.length-1);
	}
    ...
    public static void main(String[] args) {
        ...
        System.out.println("Achou:" + busca(numeros, n));
        teclado.close();
